...
export async function saveQuote(q) {
  ...
  try {
    ...
    for (const it of q.items) {
      // 상품코드 공란(업체가 품번 미기재)이면 상품명을 코드로 사용 — 서로 다른 무코드 품목이 한 행으로 병합되는 것 방지
      const code = String(it.code ?? "").trim() || String(it.name ?? "").trim();
      const { rows: [prod] } = await client.query(
        `INSERT INTO product (code, name, manufacturer, spec, spec_amount, spec_unit)
         ...
         RETURNING id`,
        [code, it.name || "", it.manufacturer || "", it.spec || "", it.specAmount, it.specUnit]
      );
      await client.query(
  ...
  const { rows } = await pool.query(`
    WITH latest AS (
      SELECT DISTINCT ON (o.product_id, q.vendor)
        ...
        q.offer_date, q.expiration_date, q.created_at AS uploaded_at
    ...
    SELECT p.code, p.name, p.manufacturer, p.spec, p.spec_amount, p.spec_unit,
           ...
           CASE WHEN p.spec_amount > 0 THEN round((l.price * 1.1 / p.spec_amount)::numeric, 2) END AS unit_price,  -- 구매가(부가세 포함) 기준 단위당(소수 2자리)
           -- 오늘 올린 견적서인지(업로드 시각 KST 기준) → 업체명에 NEW
           (l.uploaded_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date AS is_new
...
export async function getPurchases() {
  const { rows } = await pool.query(`
    SELECT id, code, name, vendor, unit_price, qty,
           ...
           -- ponytail: KST 하드코딩(랩실 단일 지역). 다른 지역 쓰게 되면 사용자 tz를 파라미터로
           (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date AS is_new
    FROM purchase
    ORDER BY purchased_at DESC NULLS FIRST, id DESC
  `);
...
// 구매 단건 복사(재구매) — 구매일·납품일만 비운 초안 행. 나머지는 그대로 복제 후 사용자가 수정
export async function copyPurchase(id) {
  const { rows } = await pool.query(
    `INSERT INTO purchase (product_id, code, name, vendor, unit_price, qty, purchased_at, delivered_at, manual)
     SELECT product_id, code, name, vendor, unit_price, qty, NULL, NULL, manual
...
export async function getSettlements() {
  const { rows } = await pool.query(
    `SELECT id, vendor, amount, settled_at, memo,
            (created_at AT TIME ZONE 'Asia/Seoul')::date = (now() AT TIME ZONE 'Asia/Seoul')::date AS is_new
     FROM settlement ORDER BY settled_at DESC, id DESC`
  );
  return rows;
}
...
// ── 사용자(로그인) ── 회원가입 없음. 관리자가 createUser로 심고, 사용자는 비번만 변경
export async function getUser(id) {
  const { rows } = await pool.query("SELECT id, pw_hash FROM app_user WHERE id = $1", [id]);
  return rows[0] || null;
}
export async function setUserPassword(id, hash) {
  await pool.query("UPDATE app_user SET pw_hash = $2 WHERE id = $1", [id, hash]);
}
export async function createUser(id, hash) {
  const { rows } = await pool.query(
    "INSERT INTO app_user (id, pw_hash) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING RETURNING id",
    [String(id).trim(), hash]
  );
