...
const UNIT = { mg: ["g", 0.001], kg: ["g", 1000], g: ["g", 1], ml: ["ml", 1], ul: ["ml", 0.001], l: ["ml", 1000], m: ["m", 1], cm: ["m", 0.01], mm: ["m", 0.001], units: ["U", 1], unit: ["U", 1], ea: ["ea", 1], "개": ["ea", 1], rxns: ["rxn", 1], rxn: ["rxn", 1] };
...
export function parseSpec(str) {
  const ms = [...String(str ?? "").matchAll(/([\d.]+)\s*(mg|kg|mm|cm|ml|ul|rxns|rxn|units|unit|l|g|m|ea|개)\b/gi)];
...
export function parseSpecFromCode(code) {
  const m = String(code ?? "").match(/-\s*(\d+(?:\.\d+)?)\s*(mg|kg|mm|cm|ml|ul|rxns|rxn|units|unit|l|g|m|ea)$/i);
